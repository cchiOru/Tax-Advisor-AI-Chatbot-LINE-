...
test('CG-17 คำสั่งระบบต้องห้ามเดาในทางที่ทำให้เสียภาษีน้อยลง', () => {
  const sm = nodeByName('AI Agent (Tax Advisor)').parameters.options.systemMessage;
  assert.ok(sm.includes('ห้ามเดาให้เสียภาษีน้อยลง'), 'ไม่พบกฎห้ามเดาเข้าข้างผู้ใช้');
...
test('CG-18 ตัวแทนทั้งสองต้องได้กฎนี้เหมือนกัน ไม่ใช่ตัวเดียว', () => {
  ...
  const b = nodeByName('AI Agent (No Memory)').parameters.options.systemMessage;
  assert.strictEqual(a, b);
  assert.ok(b.includes('ห้ามเดาให้เสียภาษีน้อยลง'));
});
...
//  ทั้งที่ฐานข้อมูลมีเงื่อนไขอายุบุตรอยู่ครบ
...
// ---------------------------------------------------------------------------
//  การเปิดเผยว่าระบบตีความโจทย์อย่างไร
// ---------------------------------------------------------------------------
//  โจทย์ภาษาคนกำกวมได้ เช่นคำว่า บุตรสอง ตีความได้ทั้ง บุตร 2 คน และ บุตรคนที่สอง
//  ถ้าระบบเลือกความหมายหนึ่งแล้วคำนวณไปเงียบ ๆ ผู้ใช้จะไม่มีทางรู้ว่าตีความผิด
//
//  เดิมแก้ด้วยการสั่งให้แบบจำลองบอกการตีความเอง ซึ่งพึ่งความร่วมมือของแบบจำลอง
//  เปลี่ยนมาให้เครื่องคำนวณสรุปจากค่าที่ได้รับจริงแล้วส่งกลับเป็นข้อความสำเร็จรูป
//  เพราะข้อความนี้สร้างจากตัวเลขที่ใช้คำนวณจริง ไม่ใช่จากความจำของแบบจำลอง
//  ตรงกับหลักการเดิมของงานนี้ที่ว่าสิ่งที่ต้องรับประกันได้ต้องบังคับด้วยโค้ด

test('CG-21 เครื่องคำนวณต้องส่งบรรทัดสรุปว่าคำนวณจากอะไรกลับมาด้วย', () => {
  const { calculateThaiPIT } = require('../n8n/tools/tax-calculator');
  const r = calculateThaiPIT({
    income: { salary: 600000 },
    allowances: { children: 2, socialSecurity: 9000 },
  });
  assert.ok(r['คำนวณจาก'], 'ไม่พบช่อง คำนวณจาก ในผลลัพธ์');
  assert.ok(r['คำนวณจาก'].includes('บุตร 2 คน'), 'ต้องระบุจำนวนบุตรที่ใช้คำนวณ');
  assert.ok(r['คำนวณจาก'].includes('ประกันสังคม'), 'ต้องระบุค่าลดหย่อนที่ใช้ทุกรายการ');
});

test('CG-22 บรรทัดสรุปต้องเผยให้เห็นเมื่อแบบจำลองใส่ตัวเลขผิดช่อง', () => {
  // เคยพบว่าโจทย์บอกประกันชีวิต 100,000 แต่แบบจำลองใส่ทั้งช่องประกันชีวิตและช่อง RMF
  // ความผิดพลาดแบบนี้เดิมซ่อนอยู่ ตรวจได้ก็ต่อเมื่อไปเปิดดูข้อมูลหลังบ้าน
  const { calculateThaiPIT } = require('../n8n/tools/tax-calculator');
  const r = calculateThaiPIT({
    income: { salary: 600000 },
    allowances: { lifeInsurance: 100000, rmf: 100000 },
  });
  assert.ok(r['คำนวณจาก'].includes('ประกันชีวิต'));
  assert.ok(r['คำนวณจาก'].includes('RMF'), 'ต้องเห็นว่ามีการใส่ RMF เข้ามาด้วย');
});

test('CG-23 คำสั่งระบบต้องบังคับให้แสดงบรรทัด คำนวณจาก และห้ามเขียนเอง', () => {
  const sm = nodeByName('AI Agent (Tax Advisor)').parameters.options.systemMessage;
  assert.ok(sm.includes('คำนวณจาก'), 'ไม่พบคำสั่งให้แสดงบรรทัด คำนวณจาก');
  assert.ok(sm.includes('ห้ามเขียนเอง'), 'ต้องห้ามแบบจำลองแต่งบรรทัดนี้ขึ้นเอง');
});
