 ...
 *
 *  กรณีทดสอบที่ 1 และ 2 ใช้ตัวเลขชุดเดียวกับตัวอย่างในฐานข้อมูลของระบบ
...
test('TC-26ก หมายเหตุมาตรา 48(2) ต้องปิดท้ายด้วยยอดที่ต้องชำระจริงเสมอ', () => {
  ...
  //   ค่าเช่า 400,000 บาท เครื่องคำนวณได้ภาษี 3,500 บาท
  //   แต่แบบจำลองตอบว่า 0 บาท โดยอ้างมาตรา 48(2) เป็นเหตุผล
  //
  // สาเหตุคือหมายเหตุเดิมเขียนว่า "คำนวณได้ 2,000 บาท ไม่เกิน 5,000 บาท
  // จึงได้รับยกเว้น" ซึ่งถูกต้องตามกฎหมาย แต่ไม่ได้ระบุว่ายกเว้นอะไร
  // สิ่งที่ยกเว้นคือวิธีคำนวณแบบขั้นต่ำ ไม่ใช่ยกเว้นภาษีทั้งก้อน
  //
  // ข้อความที่ตีความได้สองทางคือช่องโหว่ของระบบพอๆ กับตัวเลขที่ผิด
  // เพราะเครื่องคำนวณส่งข้อความนี้ให้แบบจำลองอ่านโดยตรง
  const กรณีทดสอบ = [
...
test('TC-26ข คำว่า "ได้รับยกเว้น" ต้องไม่ปรากฏลอยๆ ในหมายเหตุมาตรา 48(2)', () => {
  // คำนี้เมื่ออยู่ข้างจำนวนเงิน ทำให้เข้าใจว่าไม่ต้องเสียภาษี
  // ถ้าจะใช้ ต้องระบุชัดว่ายกเว้นเฉพาะวิธีคำนวณ
...
// ---------------------------------------------------------------------------
//  ประโยคสำเร็จรูปสำหรับตอบผู้ใช้
// ---------------------------------------------------------------------------
//  คำถามเรื่องยื่นล่าช้าตีความได้สองแบบเสมอ คือ "เสียเพิ่มเท่าไหร่"
//  กับ "เสียทั้งสิ้นเท่าไหร่" ซึ่งวัดแล้วพบว่าแบบจำลองภาษาเลือกตอบไม่คงที่
//  จึงบังคับให้เครื่องคำนวณส่งประโยคที่มีครบทั้งสองจำนวนกลับไป
//  การทดสอบกลุ่มนี้กันไม่ให้ประโยคนั้นขาดตัวเลขใดตัวเลขหนึ่งไป
...
// ---------------------------------------------------------------------------
// กลุ่มที่ 7: เพดานประกันสังคมที่เปลี่ยนตามปีภาษี
// ---------------------------------------------------------------------------
//  ที่มาของชุดทดสอบนี้
//    เพดานเงินสมทบประกันสังคมไม่ได้อยู่ในประมวลรัษฎากร มาตรา 47(1)(ฌ)
//    เขียนแค่ว่า "ตามจำนวนที่จ่ายจริง" ตัวเลขทั้งหมดมาจากกฎหมายประกันสังคม
//    ซึ่งเป็นคนละฉบับและแก้ไขบ่อยกว่ามาก
//
//    ราชกิจจานุเบกษาประกาศกฎกระทรวงเมื่อ 12 ธันวาคม 2568 ปรับเพดานแบบขั้นบันได
//    มีผล 1 มกราคม 2569 ตัวเลข 9,000 บาทที่เคยฝังไว้ในโปรแกรมจึงหมดอายุ
//    ชุดทดสอบนี้กันไม่ให้เพดานค้างอยู่ที่ค่าเดิมตอนเปลี่ยนปีภาษี
...
test('TC-45 เพดานต่อปีต้องเท่ากับเงินสมทบต่อเดือนคูณสิบสองเสมอ', () => {
  // ถ้าใครแก้ตัวเลขในตารางแล้วแก้ไม่ครบทุกช่อง ข้อนี้จะจับได้
...
test('TC-46 เงินสมทบต่อเดือนต้องเท่ากับร้อยละ 5 ของฐานค่าจ้างขั้นสูง', () => {
  // ร้อยละ 5 เป็นอัตราตามกฎหมายประกันสังคม ถ้าตารางขัดกับอัตรานี้แปลว่ากรอกผิด
  for (const r of SOCIAL_SECURITY_CAP_BY_YEAR) {
    assert.strictEqual(
      r.สมทบต่อเดือน,
...
test('TC-47 เครื่องคำนวณต้องตัดเงินสมทบส่วนที่เกินเพดานของปีภาษีที่ใช้อยู่', () => {
  // กรณีตามแนวคำตอบกรมสรรพากร จ่ายจริง 10,000 บาท แต่เกินที่กฎหมายกำหนด
...
test('TC-48 หมายเหตุของรายการประกันสังคมต้องบอกปีภาษีกำกับไว้ด้วย', () => {
  // ตัวเลขเพดานลอย ๆ ไม่มีปีภาษีกำกับ คือสาเหตุที่ทำให้ตอบผิดข้ามปี
  const r = calculateThaiPIT({
    income: { salary: 600000 },
    allowances: { socialSecurity: 9000 },
  });
