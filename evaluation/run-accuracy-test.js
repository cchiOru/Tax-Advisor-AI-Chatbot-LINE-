 ...
 *    3. จำลองการสืบค้นฐานข้อมูลด้วยตรรกะเดียวกับที่ใช้ในระบบ
...
function loadProductionConfig() {
  ...
  //
  // ฐานข้อมูลต้องรวมอยู่ในลายนิ้วมือด้วย เพราะเป็นข้อมูลที่ส่งเข้าแบบจำลองภาษาโดยตรง
  // ถ้าไม่รวม การเพิ่มฐานข้อมูลชุดใหม่แล้วรันซ้ำจะหยิบผลเดิมจากจุดบันทึกมาใช้
  ...
  // ---------------------------------------------------------------------
  // เคยเขียนไว้แค่คำสั่งระบบ เครื่องมือ คำสำคัญ และฐานข้อมูล ซึ่งไม่ครบ
...
function loadKnowledgeBase() {
  ...
  //
  // เหตุผล เคยเขียนรายชื่อไฟล์ไว้ในโค้ดแล้วลืมเติมเมื่อเพิ่มฐานข้อมูลชุดใหม่
  // ผลคือการวัดใช้ข้อมูลกฎหมายเก่ากว่าที่อยู่ในฐานข้อมูลจริง
...
async function main() {
  ...
  // ---------------------------------------------------------------------------
  //  กับดักที่เคยทำให้เสียโควตาไปฟรีหนึ่งวัน
  //
  //    ตอนโควตาหมด ระบบเคยบอกว่า "รันคำสั่งเดิมอีกครั้ง ระบบจะวัดต่อให้เอง"
  //    ซึ่งถูกเฉพาะกรณีที่ครั้งแรกไม่ได้ใส่ --fresh
  //
  //    ถ้าครั้งแรกใส่ --fresh แล้วรันคำสั่งเดิมซ้ำตามที่บอก
  //    --fresh จะสั่งให้ทิ้งจุดบันทึกแล้วเริ่มนับหนึ่งใหม่
  //    ผลที่วัดไปแล้วหายทั้งหมด และโควตาที่เพิ่งฟื้นจะถูกใช้ไปกับข้อเดิมซ้ำ
  //
  //    ข้อความแนะนำจึงต้องรู้ว่าคราวนี้ใส่ --fresh มาหรือเปล่า
  //    แล้วบอกคำสั่งที่ถูกต้องจริง ๆ ไม่ใช่บอกว่า "คำสั่งเดิม" ลอย ๆ
  // ---------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // ตัวชี้วัดที่ 3: ความครบถ้วนของฐานข้อมูล
  // -------------------------------------------------------------------------
  // คำถามที่ค้นฐานข้อมูลไม่เจอเลย แปลว่าแบบจำลองต้องตอบจากความรู้ทั่วไป
  ...
  // สรุปสามด้านที่ตกลงกันว่าใช้ตัดสิน: ความถูกต้อง ความไว ความครบถ้วนของฐานข้อมูล
