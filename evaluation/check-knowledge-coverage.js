/**
 * ตรวจความครบถ้วนของฐานข้อมูล — ไม่เรียก API ไม่มีค่าใช้จ่าย รันได้ทันทีทุกเมื่อ
 *
 * ตอบคำถามว่า "ฐานข้อมูลครบมั้ย" ด้วยหลักฐาน 4 ชุด
 ...
 *   4. หัวข้อภาษีที่คนถามบ่อยแต่ยังไม่มีในฐานข้อมูล
 *
...
// ---------------------------------------------------------------------------
// หัวข้อที่ผู้เสียภาษีถามบ่อย ใช้ตรวจว่าฐานข้อมูลมีครอบคลุมหรือยัง
...
const COMMON_TOPICS = [
  ...
  // หัวข้อที่เพิ่มพร้อมฐานข้อมูลชุดที่ 3
...
function main() {
  ...
  console.log(line());
  console.log('  ตรวจความครบถ้วนของฐานข้อมูล (ไม่เรียก API)');
  console.log(line());
  console.log(`ข้อมูลในฐานข้อมูล : ${kb.length} รายการ`);
  ...
  // ถ้าไม่ปรากฏ แปลว่าแบบจำลองต้องตอบจากความรู้ในตัวเอง ไม่ใช่จากฐานข้อมูลของเรา
  ...
  console.log(`  ฐานข้อมูลมีคำตอบให้ : ${answerRate.toFixed(1)}%  (${answered.length}/${kwQuestions.length})`);
  ...
  if (missing.length === 0 && zero.length === 0 && notAnswered.length === 0) {
    console.log('  ฐานข้อมูลครอบคลุมพอสำหรับขอบเขตที่กำหนดไว้');
  } else {
