...
/**
 * เพดานเงินสมทบประกันสังคมของผู้ประกันตนมาตรา 33 แยกตามปีภาษี
 * ----------------------------------------------------------------------------
 * ที่มาของตัวเลข
 *   ประมวลรัษฎากร มาตรา 47(1)(ฌ) ให้หักลดหย่อน "ตามจำนวนที่จ่ายจริง"
 *   โดยไม่มีเพดานอยู่ในตัวบทเลย เพดานทั้งหมดมาจากกฎหมายประกันสังคม
 *   ซึ่งกำหนดฐานค่าจ้างขั้นสูงที่ใช้คำนวณเงินสมทบ แล้วคิดร้อยละ 5 ของฐานนั้น
 *
 *   กฎกระทรวงกำหนดค่าจ้างขั้นต่ำและขั้นสูงที่ใช้เป็นฐานในการคำนวณเงินสมทบ
 *   ของผู้ประกันตนตามมาตรา 33 ประกาศราชกิจจานุเบกษา 12 ธันวาคม 2568
 *   ปรับเพดานแบบขั้นบันไดสามระยะ มีผลตั้งแต่ 1 มกราคม 2569
 *
 * เรียงจากปีใหม่ไปเก่า เพื่อให้ find() เจอช่วงที่ใช้บังคับก่อนเสมอ
 */
const SOCIAL_SECURITY_CAP_BY_YEAR = [
  { ตั้งแต่ปีภาษี: 2575, ฐานค่าจ้างต่อเดือน: 23000, สมทบต่อเดือน: 1150, เพดานต่อปี: 13800 },
  { ตั้งแต่ปีภาษี: 2572, ฐานค่าจ้างต่อเดือน: 20000, สมทบต่อเดือน: 1000, เพดานต่อปี: 12000 },
  { ตั้งแต่ปีภาษี: 2569, ฐานค่าจ้างต่อเดือน: 17500, สมทบต่อเดือน: 875, เพดานต่อปี: 10500 },
  { ตั้งแต่ปีภาษี: 0, ฐานค่าจ้างต่อเดือน: 15000, สมทบต่อเดือน: 750, เพดานต่อปี: 9000 },
];

/** คืนกฎเพดานประกันสังคมที่ใช้บังคับกับปีภาษีที่ระบุ */
function socialSecurityRule(taxYear) {
  const y = Number(taxYear) || 0;
  return SOCIAL_SECURITY_CAP_BY_YEAR.find((r) => y >= r['ตั้งแต่ปีภาษี']);
}
...
const LIMITS = {
  ...
  /**
   * เพดานเงินสมทบประกันสังคม ไม่ได้ตั้งค่าเป็นตัวเลขตายตัวเหมือนรายการอื่น
   * แต่คำนวณจากตาราง SOCIAL_SECURITY_CAP_BY_YEAR ด้านล่างตามค่า TAX_YEAR
   *
   * เหตุผล ตัวเลขนี้ไม่ได้อยู่ในประมวลรัษฎากร มาตรา 47(1)(ฌ) เขียนแค่ว่า
   * "ตามจำนวนที่จ่ายจริง" เพดานมาจากกฎหมายประกันสังคมซึ่งเป็นคนละฉบับ
   * และเปลี่ยนตามกฎกระทรวงที่ออกเป็นระยะ ต่างจากค่าลดหย่อนอื่นในตารางนี้
   * ที่ผูกกับประมวลรัษฎากรโดยตรงและเปลี่ยนน้อยกว่ามาก
   *
   * ถ้าฝังไว้เป็น 9000 ตรง ๆ วันที่เปลี่ยน TAX_YEAR เป็น 2569
   * ตัวเลขนี้จะค้างอยู่ที่ค่าเดิมโดยไม่มีอะไรเตือน
   */
  get SOCIAL_SECURITY() {
    return socialSecurityRule(TAX_YEAR).เพดานต่อปี;
  },
...
function calcAllowances(a, grossIncome) {
  ...
  const ssoCap = LIMITS.SOCIAL_SECURITY;
  const sso = Math.min(num(a.socialSecurity), ssoCap);
  if (sso > 0) {
    add('เงินสมทบกองทุนประกันสังคม', sso, `ไม่เกิน ${ssoCap.toLocaleString('en-US')} บาท ปีภาษี ${TAX_YEAR}`);
  }
...
function calculateThaiPIT(input) {
  ...
  // หมายเหตุตรงนี้ถูกส่งต่อให้แบบจำลองอ่าน จึงต้องเขียนให้ตีความผิดไม่ได้
  //
  // เหตุผล: เดิมข้อความเขียนว่า "คำนวณได้ 2,000 บาท ซึ่งไม่เกิน 5,000 บาท
  // จึงได้รับยกเว้น" ซึ่งถูกต้องตามกฎหมาย แต่ไม่ได้บอกว่า "อะไร" ได้รับยกเว้น
  // สิ่งที่ได้รับยกเว้นคือ "วิธีคำนวณแบบขั้นต่ำ" ไม่ใช่ตัวภาษีทั้งก้อน
  // ในการวัดผลรอบวันที่ 12 ส.ค. แบบจำลองอ่านหมายเหตุนี้แล้วสรุปว่า
  // ภาษีเป็นศูนย์ พร้อมอ้างมาตรา 48(2) เป็นเหตุผล ทั้งที่เครื่องคำนวณ
  // ส่งค่า 3,500 บาทมาให้ (คำถามข้อ 41 ผิด 2 ใน 3 รอบ)
  //
  // ทุกข้อความจึงต้องปิดท้ายด้วยจำนวนที่ต้องชำระจริงเสมอ
  if (incomeForMinTax >= LIMITS.MIN_TAX_THRESHOLD) {
    const computed = round2(incomeForMinTax * LIMITS.MIN_TAX_RATE);
    const ตามก้าวหน้า = `ยอดที่ต้องชำระคือ ${progressive.tax.toLocaleString('en-US')} บาท ตามวิธีอัตราก้าวหน้า`;

    if (computed <= LIMITS.MIN_TAX_EXEMPT) {
      minTaxNote =
        `วิธีคำนวณภาษีขั้นต่ำตามมาตรา 48(2) คำนวณได้ ${computed.toLocaleString('en-US')} บาท ` +
        `ไม่เกิน 5,000 บาท จึงไม่ต้องใช้วิธีนี้ (ยกเว้นเฉพาะวิธีคำนวณ ไม่ใช่ยกเว้นภาษี) ` +
        ตามก้าวหน้า;
    } else {
      minTax = computed;
      if (minTax > progressive.tax) {
        minTaxApplied = true;
        minTaxNote =
          `เงินได้ประเภท 40(2)-(8) รวม ${incomeForMinTax.toLocaleString('en-US')} บาท ` +
          `ต้องคำนวณภาษีขั้นต่ำ 0.5% = ${minTax.toLocaleString('en-US')} บาท ` +
          `ซึ่งสูงกว่าวิธีอัตราก้าวหน้า ยอดที่ต้องชำระคือ ${minTax.toLocaleString('en-US')} บาท ตามวิธีขั้นต่ำ`;
      } else {
        minTaxNote =
          `วิธีคำนวณภาษีขั้นต่ำตามมาตรา 48(2) ได้ ${minTax.toLocaleString('en-US')} บาท ` +
          `ไม่สูงกว่าวิธีอัตราก้าวหน้า จึงไม่ต้องใช้วิธีนี้ ` +
          ตามก้าวหน้า;
      }
    }
  }
  ...
  // ---------------------------------------------------------------------
  // สรุปว่าคำนวณจากอะไรบ้าง ให้แบบจำลองยกไปแสดงในคำตอบ
  // ---------------------------------------------------------------------
  // เหตุผลที่ต้องมี พบจากการใช้งานจริงว่าโจทย์ภาษาคนมักกำกวม
  // เช่นคำว่า "บุตรสอง" ตีความได้ทั้ง บุตร 2 คน และ บุตรคนที่สอง
  // ระบบเลือกความหมายหนึ่งแล้วคำนวณไปเงียบ ๆ ถ้าตีความผิดผู้ใช้จะไม่มีทางรู้
  //
  // ทางแก้ที่เชื่อถือได้กว่าการสั่งให้แบบจำลองบอกการตีความเอง
  // คือให้เครื่องคำนวณสรุปจากค่าที่ได้รับจริงแล้วส่งกลับไปเป็นข้อความสำเร็จรูป
  // เพราะข้อความนี้สร้างจากตัวเลขที่ใช้คำนวณจริง ไม่ใช่จากความจำของแบบจำลอง
  //
  // ผลพลอยได้ที่สำคัญกว่า ถ้าแบบจำลองส่งค่าผิดช่อง ผู้ใช้จะเห็นทันทีในคำตอบ
  // เช่นโจทย์บอกประกันชีวิตแต่ระบบไปใส่ช่อง RMF จะปรากฏในบรรทัดนี้ให้จับได้
  // เดิมความผิดพลาดแบบนี้ซ่อนอยู่ ตรวจได้ก็ต่อเมื่อไปเปิดดูข้อมูลหลังบ้าน
  const usedItems = [];
  for (const d of expenses.details) {
    usedItems.push(`${d['ประเภทเงินได้']} ${Number(d['เงินได้']).toLocaleString('en-US')} บาท`);
  }
  for (const d of allowanceResult.details) {
    if (d['รายการ'] === 'ค่าลดหย่อนส่วนตัว') continue; // ระบบใส่ให้ทุกคนอยู่แล้ว ไม่ต้องบอกซ้ำ
    usedItems.push(`${d['รายการ']} ${Number(d['จำนวน']).toLocaleString('en-US')} บาท`);
  }
  for (const d of donations.details) {
    usedItems.push(`${d['รายการ']} ${Number(d['จำนวน']).toLocaleString('en-US')} บาท`);
  }
  if (num(data.withholdingTax) > 0) {
    usedItems.push(`ภาษีหัก ณ ที่จ่าย ${num(data.withholdingTax).toLocaleString('en-US')} บาท`);
  }

  return {
    สำเร็จ: true,
    ปีภาษี: TAX_YEAR,
    คำนวณจาก: usedItems.join(' · ') || 'ไม่มีข้อมูลที่ใช้คำนวณ',
    สรุป: {
    ...
    // ---------------------------------------------------------------------
    // ส่งกลับเฉพาะขั้นภาษีที่มีภาษีเกิดขึ้นจริง
    // ---------------------------------------------------------------------
    // เหตุผล ผลลัพธ์ก้อนนี้ถูกส่งกลับเข้าหน้าต่างบริบทของแบบจำลองภาษาทุกครั้ง
    // ที่มีการเรียกเครื่องมือ จึงกินโควตาโทเคนเท่ากับข้อมูลนำเข้า
    //
    // วัดจากของจริง ผลลัพธ์ของโจทย์ที่มีค่าลดหย่อนหลายรายการยาว 1,605 ตัวอักษร
    // ราว 1,070 โทเคน โดยตารางขั้นบันไดภาษีกินไปเกือบครึ่ง
    // เมื่อรวมกับข้อมูลนำเข้า 7,033 โทเคน และข้อมูลที่ส่งให้เครื่องมืออีกราว 350
    // จะเกินหน้าต่างบริบท 8,192 โทเคน แบบจำลองจึงตอบกลับมาเป็นค่าว่าง
    // ซึ่งตรงกับที่วัดได้ว่าข้อที่ตอบว่างทุกข้อเป็นโจทย์คำนวณที่ซับซ้อนที่สุด
    //
    // ขั้นที่ภาษีเป็นศูนย์ไม่มีข้อมูลให้แบบจำลองใช้ตอบ ตัดออกได้โดยไม่เสียความหมาย
    // ส่วนขั้นที่ยังไม่ถึงก็ไม่ต้องส่ง เพราะไม่เกี่ยวกับผู้ใช้รายนี้
    ขั้นบันไดภาษี: progressive.breakdown.filter((b) => b['ภาษีในขั้นนี้'] > 0),
    ...
    // คำเตือนฉบับย่อ เพราะโหนดเตรียมคำตอบต่อคำเตือนฉบับเต็มให้ผู้ใช้อยู่แล้ว
    // ไม่จำเป็นต้องให้แบบจำลองอ่านข้อความยาวซ้ำอีกรอบ
    คำเตือน: 'ประมาณการตามข้อมูลที่ผู้ใช้ให้มา อัตราปีภาษี 2567 ตรวจสอบกับกรมสรรพากรก่อนยื่นจริง',
  };
}
